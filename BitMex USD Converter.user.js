        ...
        if(indexPrice != 0) {
            ...
            $('td.unrealisedPnl').each(function() {
                ...
                if(parts[1] == 'XBT' || parts[1] == 'mXBT' || parts[1] == 'XBt' || parts[1] == 'μXBT') {
                    ...
                    let newDivContent;
                    if(!isSpan) {
                        newDivContent = '$' + unrealizedPNLUSD + ' USD | ' + formatUnrealizedPNL + ' XBT ' + ' ' + parts[2];
                    } else {
                        newDivContent = '<span style="background:rgba(86,188,118,0.25);" class="' + ( formatUnrealizedPNL*indexPrice < 0 ? 'neg' : 'pos' ) + ' tooltipWrapper hovered">$' + unrealizedPNLUSD + ' USD | ' + formatUnrealizedPNL + ' XBT ' + ' ' + parts[2] + '</span>';
                    }
            ...
            $('td.combinedRealisedPnl').each(function() {
                ...
                let realizedPNLUSDhoverContent = '$' + (realizedPNLhover*indexPrice).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' USD | ' + realizedPNLhover + ' XBT';
                let realizedPNLUSDContent = '$' + (realizedPNL*indexPrice).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' USD | ' + realizedPNL + ' XBT';
